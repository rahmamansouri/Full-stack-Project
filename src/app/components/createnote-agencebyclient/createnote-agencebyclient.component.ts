...
@Component({
  selector: 'app-createnote-agencebyclient',
  templateUrl: './createnote-agencebyclient.component.html',
  styleUrls: ['./createnote-agencebyclient.component.css']
})
export class CreatenoteAgencebyclientComponent {
  submitted = false;
    ...
    if (!this.noteAgencesForm.valid) {
      ...
      return this.apiService.createnoteAgence(this.noteAgencesForm.value).subscribe({
        complete: () => {
          ...
          //  this.ngZone.run(() => this.router.navigateByUrl('/clientlistnoteagence'));
        },
        error: (e) => {
          console.log(e);
        },
